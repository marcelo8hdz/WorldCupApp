...
class Group(models.Model):
  group_ID = models.AutoField(default=dt.now, primary_key=True, editable=False)
  GROUP_A = 'A'
  GROUP_B = 'B'
  GROUP_C = 'C'
  GROUP_D = 'D'
...
class Team(models.Model):
  team_ID = models.AutoField(default=dt.now(), primary_key=True, editable=False)
...
class HistoricalGame(models.Model):
  hist_Game_ID = models.AutoField(default=dt.now(), primary_key=True, editable=False)
...
class WorldCupGame(models.Model):
    world_cup_game_ID = models.AutoField(default=dt.now(), primary_key=True, editable=False)
...
class Prediction(models.Model):
  game = models.OneToOneField(WorldCupGame, on_delete=models.CASCADE, primary_key=True, default=dt.now())
  home_win = models.FloatField(default=0)
  away_win = models.FloatField(default=0)
  draw = models.FloatField(default=0)

class User(AbstractUser):
  user_ID = models.AutoField(default=100000, primary_key=True, editable=False)
...
class Vote(models.Model):
  vote_ID = models.AutoField(default=dt.now(), primary_key=True, editable=False)
  HOME_WIN = 'H'
  AWAY_WIN = 'A'
  DRAW = 'D'
  RESULT_CHOICES = (
